    ...
    public static void main(String[] args) throws Exception {
        ...
        for (File f : files) {
            ...
            for (int i = 0; i < entries.getLength(); i++) {
                ...
                String v = mazeDeobfuscate(el.getTextContent());
                map.put(k, v);
            }

            String host = map.getOrDefault("sshServer", "");
            String port = map.getOrDefault("sshPort", "22");
            String sni = map.getOrDefault("customSNI", "");
            String payload = map.getOrDefault("proxyPayload", "");
            boolean useSsl = "1".equals(map.get("use_ssl"));
            boolean usePayload = "1".equals(map.get("use_payload"));

            String proto = "SSL / TLS";
            if (useSsl && usePayload) proto = "SSL + Payload (WebSocket)";
            else if (usePayload) proto = "HTTP DIRECT / PAYLOAD";

            // Format for connection_methods.db: name|ssh_host|ssh_port|ssh_user|ssh_pass|protocol|sni|payload
            System.out.println(name + "|" + host + "|" + port + "|||" + proto + "|" + sni + "|" + payload);
        }
    }
}
