...
public class ConfigImportFileActivity extends BaseActivity implements ManagerFilesAdapter.OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			...
			File appFolder = new File(HOME_PATH + "/SocksHttp");
			if (!appFolder.exists()) {
				appFolder.mkdirs();
			}
			fileManager(appFolder.getPath());
		}
	}
	...
	*/
	...
	public void onItemClick(View view, int position) {
		...
		if (file.isDirectory()) {
			...
			if (file_extensao != null && file_extensao.equalsIgnoreCase(ConfigParser.FILE_EXTENSAO)) {
				try {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
			try {
				...
				try {
					...
					Toast.makeText(this, "Permiso no soportado en este dispositivo", Toast.LENGTH_SHORT).show();
				}
			}
			return;
		}
