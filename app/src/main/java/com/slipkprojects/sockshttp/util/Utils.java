...
public class Utils
{
	...
	public static String getHWID(Context context) {
		android.content.SharedPreferences prefs = context.getSharedPreferences("hwid_prefs", Context.MODE_PRIVATE);
		String hwid = prefs.getString("hwid", null);
		if (hwid == null) {
			hwid = java.util.UUID.randomUUID().toString().replace("-", "").toLowerCase();
