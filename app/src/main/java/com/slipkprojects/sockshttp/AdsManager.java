...
public class AdsManager
{
	...
	private void setupAdsInterstitial() {
		...
		mInterstitialAd.setAdListener(new AdListener() {
			...
			public void onAdFailedToLoad(int errorCode) {
				...
				Log.e(TAG, "Interstitial ad failed to load with error code: " + errorCode);
			}
	...
	public void loadAdsInterstitial(boolean force) {
		// carrega anúncio a cada 1 hora ou se forzado
		long time = 60*60*1;
		if (mInterstitialAd != null) {
			if (force || ((SystemClock.elapsedRealtime() - mPrefs.getLong("last_ads_time", 0)) / 1000) >= time) {
				mInterstitialAd.loadAd(new AdRequest.Builder().build());
				Log.d(TAG, "Carregando anúncio interstitial (force=" + force + ")..");
			}
		}
	}
