...
public class ChannelManager implements MessageHandler
{
	...
	{
		return waitForGlobalRequestResult(0);
	}

	private final boolean waitForGlobalRequestResult(long timeoutMs) throws IOException
	{
		synchronized (channels)
		{
			long expiry = timeoutMs > 0 ? System.currentTimeMillis() + timeoutMs : 0;
			while ((globalSuccessCounter == 0) && (globalFailedCounter == 0))
			{
				...
				long waitTime = 0;
				if (timeoutMs > 0) {
					waitTime = expiry - System.currentTimeMillis();
					if (waitTime <= 0)
					{
						throw new java.net.SocketTimeoutException("The ping request timed out (" + timeoutMs + " ms).");
					}
				}

				try
				{
					channels.wait(waitTime);
				}
				catch (InterruptedException ignore)
				{
					throw new InterruptedIOException();
	...
	{
		return requestGlobalTrileadPing(0);
	}

	public long requestGlobalTrileadPing(long timeoutMs) throws IOException
	{
		synchronized (channels)
		{
			globalSuccessCounter = globalFailedCounter = 0;
		...
		try
		{
			if (waitForGlobalRequestResult(timeoutMs) == true) {
				long receivedMillis = System.currentTimeMillis();
				
				return (receivedMillis - sentMillis);
