...
public final class Cryptor {
    ...
    public byte[] decryptFromBase64(String encryptedBase64){
        String[] parts = encryptedBase64.trim().split(SPLITTER);
        if(parts.length != 3){
            throw new IllegalArgumentException("Malformed data string");
        }

        byte[] salt = fromBase64(parts[INDEX_SALT].trim());
        byte[] iv = fromBase64(parts[INDEX_IV].trim());
        byte[] cipherText = fromBase64(parts[INDEX_CIPHER_TEXT].trim());
