...
public class CustomNativeLoader {
    ...
    private static boolean loadFromZip(Context context, String libname, File destLocalFile, String arch) {
        ...
        try {
            ...
            ZipEntry entry = zipFile.getEntry("lib/" + arch + "/lib" + libname + ".so");
            if (entry == null) {
                entry = zipFile.getEntry("jni/" + arch + "/lib" + libname + ".so");
                if (entry == null)
                    throw new Exception("Unable to find file in apk:" + "lib/" + arch + "/lib" + libname + ".so");
            }
    ...
    public static File loadNativeBinary(Context context, String libname, File destLocalFile) {
        ...
        try {
            ...
            File fileNativeBin = new File(getNativeLibraryDir(context), "lib" + libname + ".so");
