...
public class Settings implements SettingsConstants
{
	...
	private static SecurityConfig minimumConfig;
	
	public Settings(Context context) {
		mContext = context;
		
		if (minimumConfig == null) {
		    minimumConfig = new SecurityConfig.Builder(SecurePreferences.getNativeKey()).build();
		}
	...
	*/
	...
	public boolean getHideServerMessage() {
		return mPrefs.getBoolean(HIDE_SERVER_MESSAGE_KEY, false);
	}
	
	public void setHideServerMessage(boolean is) {
		SharedPreferences.Editor editor = mPrefs.edit();
		editor.putBoolean(HIDE_SERVER_MESSAGE_KEY, is);
		editor.commit();
	}
	...
	*/
	...
	public int getSSHPinger() {
		String ping = mPrefs.getString(PINGER_KEY, "0");
