...
implements SkStatus.StateListener
{
	...
	public void onCreate()
	{
		...
		mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "SocksHttp:WakeLock");
		if (mWakeLock != null) {
			mWakeLock.acquire();
		}
	}
	...
	private void showNotification(final String msg, final String channel,
		...
		if (mNotifyBuilder == null) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
				...
				mNotifyBuilder = new Notification.Builder(this, NOTIFICATION_CHANNEL_BG_ID);
			} else {
        ...
        if (status == ConnectionStatus.LEVEL_CONNECTED) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                mNotifyBuilder.setUsesChronometer(false);
            }
        }
	...
	public static PendingIntent getGraphPendingIntent(Context context) {
        ...
        PendingIntent fpIntent = PendingIntent.getActivity(context, 0, intent, flags);
	...
	private ConnectivityManager.NetworkCallback networkCallback = new ConnectivityManager.NetworkCallback() {
		...
		public void onAvailable(Network net) {
			String netType = getNetworkTypeName();
			SkStatus.logDebug("📡 Red disponible: " + netType);
			...
			if (diag != null) {
				diag.logNetworkChange("RED DISPONIBLE", netType);
			}
		}

		@Override
		public void onLost(Network net) {
			SkStatus.logDebug("⚠️ Red perdida");
			com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics diag = 
				com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics.getInstance();
			if (diag != null) {
				diag.logNetworkChange("RED PERDIDA", "La conexión de red se cayó");
			}
		}

		@Override
		public void onUnavailable() {
			SkStatus.logDebug("❌ Red no disponible");
			com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics diag = 
				com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics.getInstance();
			if (diag != null) {
				diag.logNetworkChange("RED NO DISPONIBLE", "Sin conexión de red");
			}
		}

		@Override
		public void onCapabilitiesChanged(Network net, android.net.NetworkCapabilities caps) {
			if (caps == null) return;
			boolean hasInternet = caps.hasCapability(android.net.NetworkCapabilities.NET_CAPABILITY_INTERNET);
			boolean validated = caps.hasCapability(android.net.NetworkCapabilities.NET_CAPABILITY_VALIDATED);
			...
			String transport = isWifi ? "WiFi" : (isCellular ? "Datos Móviles" : "Otro");
			String status = "Internet=" + hasInternet + " Validada=" + validated;
			
			com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics diag = 
				com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics.getInstance();
			if (diag != null) {
				diag.logNetworkChange("CAMBIO DE RED", transport + " | " + status);
			}
		}
	};
	...
	private String getNetworkTypeName() {
		try {
			...
			if (info != null) {
				...
				if (sub != null && !sub.isEmpty()) type += " (" + sub + ")";
				return type;
			}
		} catch (Exception e) {}
		return "Desconocido";
