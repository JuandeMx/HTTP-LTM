...
public class TLSSocketFactory extends SSLSocketFactory {
    ...
    private Socket enableTLSOnSocket(Socket socket) {
        if (socket != null && (socket instanceof SSLSocket)) {
            // Enable only modern, secure TLS protocols to prevent DPI downgrade and Conscrypt exceptions
            try {
                ((SSLSocket) socket).setEnabledProtocols(new String[] {"TLSv1.2", "TLSv1.3"});
            } catch (IllegalArgumentException e) {
                // Fallback if TLSv1.3 is not supported on older Android versions
