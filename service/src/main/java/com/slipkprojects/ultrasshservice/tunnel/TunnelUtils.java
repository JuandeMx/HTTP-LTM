...
public class TunnelUtils
{
	...
	public static String formatCustomPayload(String hostname, int port, String payload, android.content.Context context) {
		...
		try {
			...
			java.lang.reflect.Method getHWID = utilsClass.getMethod("getHWID", android.content.Context.class);
			String currentHwid = (String) getHWID.invoke(null, context);
			BBCODES_LIST.put("[hwid]", currentHwid);
		} catch (Exception e) {
			BBCODES_LIST.put("[hwid]", "");
		}
		...
		if (!payload.isEmpty()) {
            payload = payload.replace("[instant_split]", "[split]");
            
            try {
                Matcher m = Pattern.compile("\\[(crlf|cr|lf|lfcr)\\*(\\d+)\\]").matcher(payload);
                ...
                while (m.find()) {
                    ...
                    for (int i = 0; i < count; i++) {
                        repeated.append("[").append(tag).append("]");
                    }
                    m.appendReplacement(sb, repeated.toString());
                }
                m.appendTail(sb);
                payload = sb.toString();
            } catch (Exception e) {}
			...
			// Auto-Detection and Cleaning for CDNs
			boolean isCloudflare = payload.toLowerCase().contains("cloudflare");
			boolean isCloudfront = payload.toLowerCase().contains("cloudfront");
			boolean isWebSocket = payload.toLowerCase().contains("websocket") || payload.toLowerCase().contains("upgrade");
			
			if (isCloudflare) {
				SkStatus.logInfo("<strong>Modo Cloudflare Activado</strong>");
			} else if (isCloudfront) {
				SkStatus.logInfo("<strong>Modo Cloudfront Activado</strong>");
			} else if (isWebSocket) {
				SkStatus.logInfo("<strong>Modo WebSocket Activado</strong>");
			}
			
			if (isCloudflare || isCloudfront || isWebSocket) {
				// 1. Remove spaces before line breaks (only trailing spaces that break HTTP specs)
				payload = payload.replaceAll("(?i)\\s+(\\[crlf\\]|\\[cr\\]|\\[lf\\]|\\[lfcr\\]|\\\\r\\\\n|\\\\n)", "$1");
				
				// Reverted aggressive [lf] standardization to preserve HTTP smuggling tricks.
			}
