...
public class TunnelVpnManager implements Tunnel.HostService
{
	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		...
		try
		{
			...
			{
				SkStatus.logError("Error crítico: startRouting devolvió falso.");
		...
		{
			SkStatus.logError("Error Fatal de VPN: " + e.getMessage());
			if (e.getCause() != null) {
				SkStatus.logError("Causa: " + e.getCause().toString());
			}
	...
	{
		SkStatus.logInfo("<strong>Túnel Conectado</strong>");
	}
	...
	{
		SkStatus.logInfo("<strong>VPN Establecida</strong>");
