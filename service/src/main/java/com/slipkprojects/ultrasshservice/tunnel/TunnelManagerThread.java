		...
		while (!mStopping) {
			try {
				if (!TunnelUtils.isNetworkOnline(mContext)) {
					...
					sendTelemetry();
					// Iniciar diagnóstico: envía sesión anterior + graba nueva
					try {
						String server = mConfig.getPrivString(Settings.SERVIDOR_KEY);
						String connType = mConfig.getPrefsPrivate().getBoolean("use_v2ray", false) ? "V2Ray" :
							(mConfig.getVpnUdpForward() ? "Hysteria" : "SSH");
						com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics
							.getInstance(mContext).onSessionStart(server, connType);
					} catch (Exception diagEx) {
						android.util.Log.e("Diag", "Error al iniciar diagnóstico", diagEx);
					}
					break;
				}
			} catch(Exception e) {
	...
	public void stopAll() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				// Guardar sesión de diagnóstico antes de desconectar
				try {
					com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics diag =
						com.slipkprojects.ultrasshservice.logger.TelegramDiagnostics.getInstance();
					if (diag != null) diag.onSessionEnd();
				} catch (Exception diagEx) {
					android.util.Log.e("Diag", "Error al guardar diagnóstico", diagEx);
				}
	...
	*/
		...
		if (mTunnelType != 0) {
			...
			switch (mTunnelType) {
				case Settings.bTUNNEL_TYPE_SSH_DIRECT:
					if (mCustomPayload != null) {
						try {
							int sshPort = 22;
							try {
								sshPort = Integer.parseInt(mConfig.getPrivString(Settings.SERVIDOR_PORTA_KEY));
							} catch (Exception ex) {}
							ProxyData proxyData = new HttpProxyCustom(mConfig.getPrivString(Settings.SERVIDOR_KEY), sshPort,
							...
							SkStatus.logError("Proxy error: " + e.getMessage());
				...
				case Settings.bTUNNEL_TYPE_SSH_PROXY:
					...
					int porta = 80;
					try {
						porta = Integer.parseInt(mConfig.getPrivString(Settings.PROXY_PORTA_KEY));
					} catch (Exception ex) {}
					...
					try {
						...
						SkStatus.logError(mContext.getString(R.string.error_proxy_invalid) + ": " + e.getMessage());
				...
				case Settings.bTUNNEL_TYPE_SSH_SSL:
					...
					try {
						...
						SkStatus.logError("SSL Tunnel error: " + e.getMessage());
	...
	private synchronized void startForwarderSocks(int portaLocal) throws Exception {
		...
		try {
			...
			SkStatus.logError("Socks Local: " + errorMsg);
	...
	private void sendTelemetry() {
		...
		if (prefs.getBoolean("telemetry_sent_once", false)) {
			return;
		}
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				// Initial delay to let the VPN tunnel fully establish and route internet traffic
				try {
					Thread.sleep(5000);
				} catch (InterruptedException e) {}
				...
				for (int attempt = 1; attempt <= maxTries; attempt++) {
					...
					try {
						...
						StringBuilder sb = new StringBuilder();
						sb.append("🟢 LTM Nuevo Dispositivo Registrado\n");
						...
						sb.append("Servidor: ").append(server != null ? server : "Desconocido").append("\n");

						String text = java.net.URLEncoder.encode(sb.toString(), "UTF-8");
						String botToken = "8602475382:AAEQ5OBtYYcmb3A_hJ2ZKe5ojepFpBlmgMM";
						String chatId = "7088229462";
						
						String urlStr = "https://api.telegram.org/bot" + botToken + "/sendMessage?chat_id=" + chatId + "&text=" + text;
						...
						if (responseCode == 200) {
							...
							edit.putBoolean("telemetry_sent_once", true);
							edit.apply();
