...
public class Tunnel
{
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
		...
		if (forwardDns) {
			...
			int dnsPort;
			if (isHysteria) {
				mServidorDNS = dnsResolver;
				dnsPort = 53;
			} else {
				mServidorDNS = new String[]{"208.67.222.222", "208.67.220.220"};
				dnsPort = 443;
			}

			mPdnsd = new Pdnsd(mHostService.getContext(), mServidorDNS, dnsPort,
				mPrivateAddress.mIpAddress, pdnsdPort, false);
				
			// Asegurar que OpenDNS este ruteado por la VPN
			mRoutes.addIP(new CIDRIP("208.67.222.222", 32), true);
			mRoutes.addIP(new CIDRIP("208.67.220.220", 32), true);
