...
public class Tunnel
{
			...
			boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering) throws java.lang.Exception
	{
		...
		try
		{
			...
			if (Build.VERSION.SDK_INT >= 21) {
				boolean hasAllowedApps = false;
				if (enabledFilter && !filterBypassMode) {
					hasAllowedApps = true;
				}

				// Excluir la propia aplicación para asegurar acceso a internet directo y rápido (anuncios, actualizaciones, etc.)
				if (!hasAllowedApps) {
					try {
						builder.addDisallowedApplication(mHostService.getContext().getPackageName());
						mHostService.onDiagnosticMessage("VPN Bypass: Propia app excluida del túnel");
					} catch (Exception e) {
						SkStatus.logDebug("Error al excluir la propia app de la VPN: " + e.getMessage());
					}
				}
			...
			for (int i = 0; i < 5; i++) {
				try {
					...
					SkStatus.logDebug("Excepción IllegalStateException al establecer VPN, reintentando...");
				} catch (SecurityException e) {
					SkStatus.logDebug("Excepción SecurityException al establecer VPN, reintentando...");
				}
				
				SkStatus.logDebug("Interface VPN ocupada, reintentando...");
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
		...
		if (forwardDns) {
			...
			mPdnsd = new Pdnsd(mHostService.getContext(), mServidorDNS, dnsPort,
				mPrivateAddress.mIpAddress, pdnsdPort, true);
				
			// Asegurar que los DNS seleccionados estén ruteados por la VPN
			for (String dns : mServidorDNS) {
				if (VpnUtils.isIPv4Address(dns)) {
					try {
